...
try {
    ...
    $lastVersion = (Find-Module $env:ModuleName -ErrorAction SilentlyContinue).Version
    $releaseNotes = (Get-Content .\$moduleName\ReleaseNotes.txt -Raw).Replace("{{NewVersion}}",$newVersion)
    $releaseNotes = $lastVersion ? $releaseNotes.Replace("{{LastVersion}}","$lastVersion") : $releaseNotes.Replace("{{LastVersion}}","")
    #endregion
    ...
    $t.package.metadata.description = (Get-Content .\$moduleName\description.txt -raw).ToString()
